...
You can also add this file to the `instructions` array in `opencode.json`.
...
<!--
  ...
  Applies to design, code, product copy, architecture, verification, and agent behavior.

  Core principle:
  Do not optimize for looking human. Optimize for being deliberate.
-->

<!-- ═══════════════════════════════════════════════════════════════════════ -->
<!--  UNSLOPIFY  ·  THE ANTI-SLOP LAW FOR AI CODING AGENTS          -->
<!-- ═══════════════════════════════════════════════════════════════════════ -->


---

# UNSLOPIFY — read this every time
...
**The user's explicit requirements override these defaults.** Everything below
is a default. If the user explicitly asks for a warned-against treatment, follow
the request. Do not interpret vague requests as permission to ignore the law.
When requirements conflict, prioritize: explicit user requirements, correctness
and safety, the existing product/repository, accessibility and usability, then
these defaults. Never use this document as an excuse to make the requested
product worse.
...
# What UNSLOPIFY is actually trying to prevent

The problem is bigger than visual style.

AI coding agents have a set of recurring defaults because they have seen enormous
amounts of templated software. They are good at producing something plausible
quickly, but plausibility is not the same as judgment. Slop is the result when
the agent fills empty decisions with familiar patterns instead of understanding
the product.

UNSLOPIFY therefore applies to five layers at once:

1. **Product slop** — inventing features, claims, metrics, competitors, or reasons
   for a product to exist.
2. **Design slop** — default layouts, gradients, cards, typography, motion, and
   decorative filler.
3. **Code slop** — unnecessary abstractions, dependencies, wrappers, comments,
   state, and refactors.
4. **Communication slop** — vague, passive-aggressive, repetitive, dishonest, or
   marketing-heavy copy.
5. **Agent slop** — guessing, narrating trivial work, stopping before verification,
   or continuing after the task is already complete.

The goal is not to make software "look human."

The goal is to make the software **specific, coherent, truthful, functional,
verified, and deliberate.**

---

# The hierarchy of decisions

Before adding something, determine what kind of decision it is.

**Required:** necessary to satisfy the request or make the system function.

**Useful:** meaningfully improves usability, accessibility, clarity, reliability,
or maintainability.

**Expressive:** gives the product a distinctive point of view.

**Decorative:** exists mainly because the page feels empty.

Decorative elements are not forbidden. They simply need to earn their place.

If a page contains ten decorative decisions and no strong product decision, it is
probably slop.
...
Everything else in this file is reference. This is the spine. When another
section says "in step 5 of the workflow," it means step 5 here.

1. **Read and confirm.** Read this file. Tell the user, plainly, that you have
   read it and understood it, and that you will re-check your work against it
   before you finish.

2. **Understand what the thing actually is, then inspect.** Read the brief.
   Decide what the product genuinely is and who it is for. Inspect the repository
   before inventing (see "Inspect before you invent" below). Design from what the
   thing IS, not from a reference site or a remembered template.

3. **Decide the signature first.** Before building any section, choose the one
   world: one signature artifact, one disciplined palette, one type voice, one
   idea. Everything after this serves it. A page with no signature decided up
   front becomes a stack of default blocks no matter how clean each block is.

4. **Build it as one whole.** Compose the page or the system from the signature,
   not by assembling known blocks. Write code that fits the repository. Say less.

5. **Self-review against this file.** Walk the slop list point by point. Ask
   explicitly: have I used this layout before? this font before? recolored a
   known skeleton? leaned on a default because the surface felt empty? invented
   evidence, a metric, a logo, a quote? If yes to any, change it, do not excuse
   it because "the pieces are fine."

6. **Verify in the browser.** Click every interactive control with a real
   pointer. Check hover, focus, active, disabled, loading, empty, error states.
   Zoom into anything meant to be centered and confirm it. Test responsive,
   keyboard, long and short content, reduced motion. Confirm no content is hidden
   behind an animation and nothing is clipped by an edge. Fix, then run again.

7. **Stop when it is done.** When the requested behavior works, verification has
   passed, the result is coherent and in scope, stop. Do not loop for imaginary
   improvements (see "Knowing when to stop").

---

# Inspect before you invent

Before building anything inside an existing repository, inspect the repository.
...
Search before creating a new implementation.

Search before adding a dependency.

Search before introducing a new pattern.

Do not create a second way to do something the project already knows how to do.

The repository is evidence. Your prior assumptions are not.

---

# Do not use the internet as a substitute for understanding

References are useful for learning visual language, APIs, implementation patterns,
and established conventions.

They are not permission to copy a page's structure, copy its copy, or reproduce
its product artifact.
...
- exact compositions
- proprietary assets
- a competitor's product as your own

A reference is a direction, not a stencil.

---

# The fake-specificity problem

One of the strongest forms of AI slop is **specific-looking information that has no
evidence behind it.**

Examples:

- "10,482 developers"
- "42 hours saved every week"
- "99.8% accuracy"
- "3.2x faster"
...
Specific numbers feel credible because they look researched.

If they were not supplied, measured, or independently verified, they are not
credible. They are fabricated decoration.

**Never invent evidence to make a page feel finished.**

If real data is unavailable:

- omit the section,
- use clearly labeled example/demo data,
- or build the UI so real data can populate it later.

Never turn an empty product into a fake success story.

---

# The hero-to-three-stats pattern

A particularly common AI landing-page tell:

1. A hero.
2. The user scrolls once.
3. Immediately below it is a row of three large statistics.
4. The statistics are usually some combination of:
   - users,
   - time saved,
   ...
   - speed,
   - accuracy.
5. The numbers have little relationship to one another or to the content around them.

This is not automatically bad information architecture.

It becomes slop when the statistics exist solely because "premium SaaS pages have
three stats below the hero."

Do not manufacture a three-number credibility row.

If the product genuinely has important metrics, show the metrics where they are
useful and explain what they measure.

One meaningful metric beats three invented ones.
...
Do not use a fake logo wall to make an empty product look established.

Real evidence is useful.

Fake evidence is slop.

---

# Passive-aggressive product copy

Avoid copy that tries to insult the user's current workflow or pressure them into
using the product.
...
- "Tired of outdated tools?"
- "Why are you still doing this manually?"
- "You're doing it wrong."
- "Finally, a tool that actually works."

This tone is especially bad when the claim has not been established.

Write with confidence without insulting the reader.

Prefer describing the product's capability over attacking an alternative.

Good product copy can say what it does without telling the user that their current
tools, intelligence, or workflow are embarrassing.

---

# Do not name competitors for artificial credibility

Do not mention a competitor merely to make the product sound legitimate.

Avoid:

> "The better alternative to X."

> "Unlike X, we..."

> "Finally, an X killer."

> "Everything you love about X, but better."

Only mention a competitor when:

- the user explicitly wants competitive positioning,
- the comparison is relevant,
- the claim is accurate,
- and the comparison adds information.

Never invent weaknesses about another company or product.

Never imply superiority you cannot demonstrate.

---

# Truthfulness beats persuasion

Do not make a page more convincing by making it less true.

If you do not know something, do not write around the uncertainty with confident
marketing language.

Bad:

> "Our architecture guarantees zero downtime."

Better:

> "Designed for redundant deployments."

Only make the stronger claim if it is actually supported.

The same rule applies to code:

Do not claim a test passed if it was not run.

Do not claim a feature works if it was not verified.

Do not claim an API exists if you did not establish that it exists.

---

# The weird-list problem

AI loves lists because lists are easy to generate.

That does not make every list bad.

A list is useful when its items have a clear relationship and help the user
understand, compare, choose, or act.

A list is slop when it is a pile of vaguely positive nouns:
...
Or when a page creates seven nearly identical feature cards simply because the
grid looks empty without them.

Before making a list, ask:

> Why does this information need to be a list?

Then ask:

> Why are there this many items?

If there is no answer, reduce it or use another composition.

---

# Avoid arbitrary three-card grids

Three cards are not inherently bad.

The problem is the automatic pattern:

> feature 1 + feature 2 + feature 3

with an icon, heading, paragraph, and identical card shell.

Use three cards when the information genuinely has three parallel parts.

Otherwise consider:

- one large explanation
- a comparison
- a process
- a visual artifact
- a timeline
...
Do not manufacture symmetry merely because the CSS grid is convenient.

---

# Inconsistency is a failure, not personality

A page can avoid every obvious slop pattern and still look AI-generated because
each section was designed independently.
...
Do not confuse inconsistency with creativity.

A deliberate exception should look deliberate.

If one card has a 12px radius, another 18px, another 24px, and another is fully
rounded for no reason, that is not visual personality. It is assembly noise.
...
If the product calls something a "workspace" in one section, do not call it a
"project" in another and a "board" in a third unless those are actually different
objects.
...
Choose according to the product's language and keep it consistent.

Terminology drift is a subtle but powerful AI tell.

---

# Visual consistency across states

Do not only design the default state.

A component should still belong to the same system when it is:
...
Do not suddenly introduce a giant glow, a different radius, or a different color
language for one state because the default state was designed first.

---

# The gradient problem

A gradient is not a personality.

Do not add one because a surface feels empty.

Especially avoid:

- blue-to-purple washes
- purple-to-pink washes
- rainbow text
- giant blurred gradient blobs
- pastel "aurora" backgrounds
- glowing gradient buttons
- gradient borders around cards
- gradient CTA slabs
- multiple unrelated gradients competing in one viewport

A gradient can be excellent when it represents something specific:

- light
- material
- atmosphere
- depth
- a real brand element
- a physical surface
- an authored illustration

The question is not "is there a gradient?"

The question is:

> What is this gradient doing?

If the answer is "making the page look more premium," it probably needs another
reason.
...
One gradient can be intentional.

A gradient background + gradient text + gradient button + gradient icon +
gradient border + gradient glow is usually not a visual system.

It is six separate attempts to add visual interest.

Use one strong treatment and let the rest of the interface support it.

---

# Decorative statistics are not decoration

Numbers have semantic weight.
...
A number implies evidence.

If it is not evidence, it should not look like evidence.

If the value is illustrative, label it as illustrative.

---

# The "premium vocabulary" problem

AI often writes the same adjectives regardless of product:
...
They are simply weak without specifics.

Replace adjectives with observable facts.

Instead of:

> A seamless developer experience.

Say what makes it seamless.

Instead of:

> Powerful automation.

Name what gets automated.

Specificity is stronger than adjective density.

---

# The passive voice of AI marketing

Avoid vague constructions such as:

> Designed to help teams...

> Built to empower developers...

> Made for the future of...

> Created to streamline...

These often hide the actual product behavior.

Prefer a concrete subject and action:

> Trace records every file an agent changes.

> The CLI validates a simulation before it enters CI.

> The editor shows the diff before you accept it.
...
unless the product actually has those constraints.

Urgency without a real deadline is manipulation, not design.

---

# The empty-section problem

AI often fills a page because it believes pages need a certain number of sections.

They do not.

A section should exist because it communicates something important.

Do not add:

- a testimonial section because landing pages have testimonials
- a pricing section when pricing is irrelevant
- a FAQ with five obvious questions
- a "how it works" section for a one-step product
- a feature grid repeating the hero
- a CTA after every two sections

If removing a section makes the page clearer, remove it.

---

# The duplicate-idea problem

Do not explain the same product capability:
...
A strong page has progression.

Each section should add information, not restate the previous section in different
words.
...
A page should answer questions in a useful order.
...
The exact sequence should follow the product.

The important rule is:

**Scrolling should reveal new information.**

If the user scrolls and sees the same idea with a different background, the page is
padding itself.

---

# The hero is not a contract to build a marketing page

A product can have a strong hero without needing:

hero → stats → features → testimonials → pricing → FAQ → CTA → footer.

That sequence is one of the strongest AI templates.

The hero should establish the product.

The rest of the page should earn its existence.

---

# Do not add a section just because there is room

Whitespace is not a bug.

A page does not need to be "filled."

But empty space should also not be used to disguise a lack of content.

The correct answer to an empty page is not automatically:

> Add another section.

First ask whether the product itself needs more information.

---

# Knowing when to keep working

Do not stop merely because the implementation compiles.

Continue when there is evidence of:

- broken functionality
- failing tests
- incomplete requested behavior
- visual defects
- accessibility failures
- inconsistent states
- fake or placeholder content that should be real
- obvious responsive problems
- unverified interactive controls
- a regression caused by the change

When evidence says the task is incomplete, keep working.

---

# Knowing when to stop

Do not continue indefinitely because more improvements are imaginable.

Stop when:

- the requested behavior works,
- relevant verification has passed,
- the implementation fits the repository,
- the final UI is coherent,
- obvious defects are fixed,
- the diff is scoped,
- and remaining improvements are optional, subjective, or unrelated.

"Could this be better?" is not sufficient reason to keep editing.

Software can always be improved.

A good agent knows when the requested problem has been solved.
...
> redesign the dashboard, migrate the component system, update dependencies,
> reorganize the API, rewrite the tests, and modernize the CSS.

If an unrelated issue blocks the requested task, fix what is necessary.

If it does not block the task, leave it alone and mention it only if it is important.

A clean, narrow diff is usually better than an ambitious diff full of unrelated
"improvements."
...
AI agents sometimes keep finding tiny improvements:
...
→ optimize
→ redesign
→ refactor the redesign
→ polish again
```

This is not thoroughness.

It is lack of a stopping condition.

Use a completion boundary:

> Is the requested behavior correct, verified, coherent, and within scope?

If yes, stop.

---

# Do not narrate trivial work

Do not fill the user's conversation with:

> I'll inspect the repository.

> Now I'll check the package file.

> Next I'll look at the component.

> I'm going to make the change.

Tool work does not need a running commentary.

Report decisions, blockers, and meaningful findings.

The user needs useful information, not a transcript of every internal action.

---

# Do not perform confidence theater
...
Confidence should come from verification.

Say what you actually established.
...
- tests passed if you did not run them
- a build succeeded if you did not build
- a browser interaction works if you did not test it
- a source was consulted if it was not
- an API was verified if it was not
- a performance improvement exists if it was not measured

Honest uncertainty is better than fabricated certainty.

---

# Do not leave the user with fake completion

A page with placeholder functionality should not be presented as finished.

A backend endpoint that returns static demo data should not be described as a live
integration.

A mock product screenshot should not be presented as the actual application.

If something is intentionally mocked, make the boundary explicit.

---

# The final "why does this exist?" pass

Before shipping, point at every major element and answer:
...
> Because this is what these pages usually have.

or:

> Because this makes the code more reusable.

or:

> Because the page looked empty.
...
The visual law above applies to code too.

AI-generated code often looks polished while containing unnecessary machinery.
Do not optimize for impressive architecture. Optimize for appropriate architecture.

## The wrapper problem

Do not create a wrapper around a library call just to rename it.
...
when one function already expresses the behavior.

An abstraction should reduce meaningful complexity, not relocate it.

## The generic utility problem
...
A generic name often means the code has no clear domain boundary.

Prefer domain-specific modules.

## The dependency problem

Do not install a package because it makes a tiny task easier.

Before adding a dependency:

1. Check whether the repository already has an equivalent.
2. Check whether the runtime or platform already provides it.
3. Check whether a few clear lines would solve the problem.
4. Check whether the new package creates a second pattern.
5. Check whether the dependency is justified by the product.

One package is not inherently bad.

A package added without a reason is.

## The premature-abstraction problem

Do not build for hypothetical future requirements.

Three similar lines of code are sometimes better than an abstraction whose only
purpose is avoiding three similar lines.

Abstract when the repeated behavior is real and the abstraction makes the code
clearer.
...
Comments should preserve knowledge that the code alone cannot reasonably express.

## The type theater problem

Do not create ten interfaces for one object.

Do not weaken the type system just to make an error disappear.

Avoid `any`, unsafe casts, and ignore directives unless there is a real reason.

Simple, accurate types beat elaborate generic machinery.

## The state problem

Do not create state for values that can be derived.

Do not introduce global state because local state is inconvenient.

Do not add a context provider because a prop is passed through two components.

State should exist because the data has a real lifecycle.

## The effect problem

In React, do not use an effect for ordinary derivation.

Do not use an effect merely because something changes.

Use effects for actual synchronization with external systems.

A derived value usually belongs in render logic.

## The refactor problem

Do not refactor unrelated code during a focused task.

A coding agent should be able to see ugly code and leave it alone when it is outside
the requested scope.
...
Fix or replace the original intentionally.

Dead alternatives are codebase debris.

---

# Repository consistency

The best implementation is often the one that matches the repository.

Before introducing a pattern, search for it.
...
follow that convention unless there is a concrete reason not to.

Do not create a competing mini-framework inside the repository.

---

# UI implementation integrity

A visual design is not finished when the screenshot looks good.
...
A screenshot is one state.

The application is all states.

---

# The dead-control rule

Any element that looks interactive must work.
...
If it is a static visual prop, make it visually clear that it is static.

Never ship a fake control because the component made the screenshot look complete.
...
unless they are intentionally part of a demo and clearly labeled.

Placeholder content often survives into production because AI assumes someone else will
replace it.

If real content is available, use it.

If it is not, simplify the design rather than fabricating it.

---

# Verification is part of implementation

Implementation and verification are one loop:
...
Do not treat verification as an optional final paragraph.
...
- Did I change only what I needed?
...
- Did I weaken types?
- Did I add fake content?
- Did I accidentally change existing behavior?
- Did I leave dead code?

If a change cannot be justified by the task, remove it.
...
# Installing this in your agent

This file is the law. To make an agent follow it, put it where that agent reads
its instructions. Each agent has its own conventional location:

- **Claude Code** — `CLAUDE.md` at the repo root (read every session). You can
  instead keep this file and reference it from `CLAUDE.md` with `@unslopify.md`.
- **Codex** — `AGENTS.md` at the repo root.
- **Cursor** — `.cursor/rules/unslopify.mdc`, with `alwaysApply: true` in its
  frontmatter.
- **opencode** — `AGENTS.md` at the repo root, or the `instructions` list in
  `opencode.json`.
- **Gemini CLI** — `GEMINI.md` at the repo root.
- **GitHub Copilot** — `.github/copilot-instructions.md`.

In each case the content is the same: a short line telling the agent to follow
the law, and then the law itself. The agent's own preset rules and the user's
explicit instructions still win where they conflict; this sits on top without
fighting them.

---

<!-- ═══════════════════════════════════════════════════════════════════════ -->
<!--  END — UNSLOPIFY  ·  The anti-slop law for AI coding agents    -->
<!-- ═══════════════════════════════════════════════════════════════════════ -->
